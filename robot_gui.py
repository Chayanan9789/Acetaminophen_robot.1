...
class RobotController:
    ...
    # <<< [เปลี่ยนแปลง] >>> เพิ่มตัวแปรสำหรับกำหนดตำแหน่งวางวัตถุ (Base)
    # แก้ไขค่ามุมตรงนี้ได้เลย (หน่วยเป็นองศาที่กล้องต้องมองไป)
    ...
    BASE_TO_CAMERA_OFFSET_DEGREES1_DROP = -7

    def __init__(self):
        self.running = True
        self.cap = None 
        # <<< [เปลี่ยนแปลง] >>> ลบ self.base_drop_angle ที่ไม่ได้ใช้ออกไป
    ...
    def pickup_sequence(self):
        ...
        self.move_gripper_smoothly(self.GRIPPER_OPEN_ANGLE)
    ...
    # <<< [เปลี่ยนแปลง] >>> แก้ไขฟังก์ชันนี้
    def search_for_object(self, target_label):
        # ถ้าเป้าหมายคือ "Base" ให้ใช้ค่าที่กำหนดไว้ล่วงหน้าทันที
        if target_label == "Base":
            ...
            # หมุนกล้องไปยังตำแหน่งของ Base เพื่อให้ผู้ใช้เห็นภาพ
            self.move_camera_smoothly(self.BASE_DROP_TARGET_ANGLE)
            time.sleep(0.5) # รอให้กล้องนิ่ง
            return True, self.BASE_DROP_TARGET_ANGLE

        # การค้นหาวัตถุอื่นๆ (ที่ไม่ใช่ Base) ยังทำงานเหมือนเดิม
        ...
        for angle in scan_angles:
            ...
            for box in results[0].boxes:
                ...
                if detected_label == target_label and confidence >= self.CONFIDENCE_THRESHOLD:
                    ...
                    if frame_width * 0.1 < center_x < frame_width * 0.9:
    ...
    def run_main_sequence(self):
        try:
            ...
            time.sleep(0.5) # ให้เวลากล้องในการเตรียมตัว
            ...
            for obj_to_find in object_sequence:
                ...
                # <<< [เปลี่ยนแปลง] >>> เรียกใช้ฟังก์ชันเดิม แต่ตอนนี้มันจะคืนค่าคงที่ออกมา
                found, drop_base_angle = self.search_for_object("Base")
                
                if not found: # ในทางปฏิบัติ ส่วนนี้จะไม่เกิดขึ้นถ้า Base ถูกกำหนดค่าไว้แล้ว
            ...
            # <<< [เปลี่ยนแปลง] >>> เรียกใช้ฟังก์ชันเดิมอีกครั้งสำหรับขั้นตอนสุดท้าย
